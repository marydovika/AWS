...
String DataLogger::getValueFromLog(String logLine, String label) {
    ...
    String parsedValue = logLine.substring(startIndex, endIndex);
    parsedValue.trim(); // ADJUSTMENT: Remove trailing spaces, carriage returns, and newlines
    
    if (parsedValue.length() == 0) return "0"; // ADJUSTMENT: Fallback if string becomes blank
    return parsedValue;
}
